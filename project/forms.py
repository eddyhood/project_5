...
class RegisterForm(FlaskForm):
    username = StringField(
        ...
        validators=[
            DataRequired(),
            Regexp(r'^[a-zA-Z0-9_]+$',
                   message=('Letters, numbers, and _ only')), name_exists])
    email = StringField(
        'Email',
...
class AddEntryForm(FlaskForm):
    ...
    time = IntegerField(
        'Time Spent',
        validators=[
        DataRequired(message='Enter a number value only for time spent')])
...
class DeleteEntryForm(FlaskForm):
    confirm_message = StringField('Type title of journal entry to delete',
